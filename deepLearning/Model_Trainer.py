...
data=pd.read_csv("C:\\Users\\Atharv Chandane\\Desktop\\Text_Summarizarion\\Dataset\\Reviews.csv",nrows=1000)
...
def text_cleaner(text):
    newString = text.lower()  # Converts the text to lowercase.
    newString = BeautifulSoup(newString, "lxml").text  #Uses BeautifulSoup to remove any HTML tags from the text.
    newString = re.sub(r'\([^)]*\)', '', newString)  #Removes text within parentheses.
    newString = re.sub('"','', newString)  #Removes double quotes.
    newString = ' '.join([contraction_mapping[t] if t in contraction_mapping else t for t in newString.split(" ")])  #Changes contracted words  
    newString = re.sub(r"'s\b","",newString)  # Removes possessive endings.
    newString = re.sub("[^a-zA-Z]", " ", newString)  #Removes non-alphabetic characters.
...
###################################################################################################
# Defining maximum lengths for text and summary
max_len_text=80 
max_len_summary=10

# Splitting Dataset into Train and Test sections
...
x_tr    =   tf.keras.preprocessing.sequence.pad_sequences(x_tr,  maxlen=max_len_text, padding='post') 
x_val   =   tf.keras.preprocessing.sequence.pad_sequences(x_val, maxlen=max_len_text, padding='post')
...
#padding zero upto maximum length
y_tr    =   tf.keras.preprocessing.sequence.pad_sequences(y_tr, maxlen=max_len_summary, padding='post')
y_val   =   tf.keras.preprocessing.sequence.pad_sequences(y_val, maxlen=max_len_summary, padding='post')

y_voc_size  =   len(y_tokenizer.word_index) +1


###################################################################################################
# 3 Stacked LSTM for Encoder
K.clear_session() 
latent_dim = 500 
...
decoder_hidden_state_input = tf.keras.layers.Input(shape=(max_len_text,latent_dim))
...
def decode_sequence(input_seq):
    ...
    while not stop_condition:
        ...
        if sampled_token_index == 0:
            continue  # Skip generating token for padding
